...
clear trial_info
clear mynumcell
%based on kuppuseqgen_2A2C2E2R_seqgen
nblocks=8;%This version updated 13 Feb 2018 by DVM Bishop
%This presents just one grammatical sequence for two sequence types, and 2 random
nsets=5;

breakblock=7;% currently set to take breakblock and adjacent block as random
myroutine=4;%can alter this
...
triplets={
    1, 'A1', 'S1', 'B1';
    2,'A1','S1','B1';   %triplet 1 and 2 are identical: twice as many presented
    3, 'C1', 'S4', 'D1';
    4,  'C1', 'S4', 'D2';
   5, 'Y1', 'R', 'R'; 
    6, 'Y2', 'R', 'R';%these will be totally random, ie identity of Y element varies
    };
ntriplet=length(triplets);
settings.triplts4set=ntriplet; %for compatibility with Mihaela
nnonrandom=3;%change this if more or less triplets

settings.stms.dependencies.fnames={'A1',  'B1', ...
    'C1', 'D1', 'D2'}';%%%%%%%%%%%% altered feb 2018!

dis_pool={ 'B1', 'D1', 'D2'}';%%%%%%%%%%%% altered feb 2018!
...
allnum=[1:7,1001:1002,10001:10080]';%%%changed bcs need numbers for unused wds that feature in practice
...
conflictfile='all_stimuli_routines_feb2018.xlsx';
...
%change the unused types to randoms - changed for latest version
%nb E1 and F1 are also now unused, but we just ignore them to keep program
%happy
unused={  'A2', 'S2', 'B2',...
    'A3','S3',  'B3',...
    'C2', 'S5', 'D3','D4',...
...
settings.stms.random.fnames=randoms;
settings.stms.dependencies.n=5;%specific for current
...
settings.stms.dependencies.fnames={'A1',  'B1',   'C1', 'D1', 'D2'}';
settings.stms.random.fnames=randoms;
settings.stms.planned.fnames=smids';
settings.stms.random.n=length(randoms);
settings.stms.dependencies.n=length(settings.stms.dependencies.fnames);
...
settings.triplts4set=20;%number of practice items to present
...
allstim=[settings.stms.dependencies.fnames;smids';randoms];
allnum=[1:5,1001:1003,10001:10080]';%%%changed bcs need numbers for unused wds that feature in practice
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
nonrandoms=[2:4] %triplets with unique nonrandom targets
targetpool=[triplets(nonrandoms,4)];%pool of targets to include in distractor set: check this!
%!!!!!! this will change according to total N triplets ; 
initialpool=triplets([1,3],2);%pool of initial items, A1, C1
...
for b = 1:nblocks
    for s = 1:nsets
        ...
        bs=b*s;%this equals one for first set and first block
        ...
        while(realnono==1)
            ...
            mynono=1;%the realnono condition added bcs of problems across blocks
            
            while(mynono==1)
                %this has to change to take into account duplicated items
                myx=[1,2,3,4,5,6]; %myx2 must not follow equiv item in myx
                myx2=[2,1,4,3,6,5];
                mynono=0;realnono=0; %if this remains at 0, then no problems with sequence
                for jj=1:length(myx)
            ...
            while char(triplets{thisi_seq(1),2})==lastt1
                ...
                tempseq=thisi_seq;
                thisi_seq=fliplr(tempseq);%reverse order of seq to fix this
                %but now we have duplicates in triplet list so we can *still* have
                %problems and loop will never end.
                if(char(triplets{thisi_seq(1),2})==lastt1)
                    realnono=1;
                    thisi_seq=[6,6,6,6,6,6];%need to start again with another seq
                    %previously this set to 8 (better to do this with
                    %repmat?
                end                
            end           
        end
...
allmystim=[];
for k=1:40 %this saves stimulus sequences for easy checking, with numeric
...
initialpool=triplets([1,2],2)%pool of initial items, A1, C1
    randoms=randoms(randperm(length(randoms)));%randomise order of random items
    ...
    for b = 1:nblocks
        for s = 1:nsets
            ...
            while(mynono==1)
                ...
                myx=[3,4]; %myx2 must not follow equiv item in myx
                myx2=[4,3];
